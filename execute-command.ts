 ...
 *    supabase login
 * 
 * 3. Initialize a new Supabase Edge Functions project or navigate to your existing one:
 *    supabase init
 * 
 * 4. Create this edge function:
 *    supabase functions new execute-command
 * 
 * 5. Save this file as 'execute-command.ts' in the 'supabase/functions/execute-command/' directory
 * 
 * 6. Set up the required secrets (environment variables):
 ...
 * 
 * NOTE: The --no-verify-jwt flag is used because we're implementing our own authentication via API key.
 *       In a production environment, you might want to use both JWT and API key authentication.
 * 
 ...
 *   -d '{"command": "SELECT * FROM your_table LIMIT 5", "type": "sql"}'
 * 
 * Important: For the 'execute_sql_as_super_user' RPC function to work, you must create it in your database.
 * Create this function in the SQL editor of your Supabase project:
 * 
